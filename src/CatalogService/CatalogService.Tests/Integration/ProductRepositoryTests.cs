// <copyright file="ProductRepositoryTests.cs" company="PlaceholderCompany">
...
public class ProductRepositoryTests
{
    ...
    public async Task ProductRepository_CRUD_And_Filtering_Pagination_Work()
    {
        ...
        var cat1Products = (await repo.GetAllAsync(cat1.Id, 1, 10).ConfigureAwait(false)).ToList();
        cat1Products.Should().HaveCount(2);

        // Pagination: pageSize=1 should return 1 item for page 2
        var page2 = (await repo.GetAllAsync(null, 2, 1).ConfigureAwait(false)).ToList();
        page2.Should().HaveCount(1);

        // GetById
        var fetched = await repo.GetByIdAsync(p2.Id).ConfigureAwait(false);
        fetched.Should().NotBeNull();
        fetched!.Name.Should().Be("P2");

        // Update
        fetched.Name = "P2-upd";
        await repo.UpdateAsync(fetched).ConfigureAwait(false);
        var updated = await repo.GetByIdAsync(fetched.Id).ConfigureAwait(false);
        updated!.Name.Should().Be("P2-upd");

        // Delete
        await repo.DeleteAsync(p1.Id).ConfigureAwait(false);
        var afterDelete = await repo.GetByIdAsync(p1.Id).ConfigureAwait(false);
